...
MyOpenGLWidgegt::MyOpenGLWidgegt(s21::Controller * controller_,QWidget *parent) : controller(controller_), QOpenGLWidget {parent} {
...
void MyOpenGLWidgegt::drawLines() {
    ...
    qDebug() << controller->get_matrix_facetes().size();
    for (size_t i = 0; i < controller->get_matrix_facetes().size(); i++) {
        glBegin(GL_LINE_STRIP);
           for (size_t j = 0; j != controller->get_matrix_facetes()[i].size(); ++j) {
            glVertex3d(controller->GetMatrixVertexes()[controller->get_matrix_facetes()[i][j] - 1][0],
                      controller->GetMatrixVertexes()[controller->get_matrix_facetes()[i][j] - 1][1],
                      controller->GetMatrixVertexes()[controller->get_matrix_facetes()[i][j] - 1][2]);
           }
             glEnd();
    }
}
